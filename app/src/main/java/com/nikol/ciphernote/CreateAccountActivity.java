...
public class CreateAccountActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        createAccountButton.setOnClickListener(v -> {
            ...
            RegisterRequest registerRequest = new RegisterRequest(username, passwordHash);

            RetrofitClient.getApiService().register(registerRequest).enqueue(new Callback<>() {
                @Override
                public void onResponse(@NonNull Call<AuthResponse> call, @NonNull Response<AuthResponse> response) {
                    if (response.isSuccessful() && response.body() != null) {
                        ...
                        new Thread(() -> {
                            ...
                            new_profile.password_hash = passwordHash;
                            new_profile.token = auth.token;
                            new_profile.keySalt = auth.keySalt;
                        ...
                        runOnUiThread(() -> {
                            Toast.makeText(CreateAccountActivity.this, "Registration failed or user already exists!", Toast.LENGTH_SHORT).show();
